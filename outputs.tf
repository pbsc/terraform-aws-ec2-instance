locals {
  ...
  this_public_ip                    = "${split(",", var.allocate_eip ? join(",", compact(concat(aws_eip.this.*.public_ip))) : join(",", compact(concat(coalescelist(aws_instance.this.*.public_ip, aws_instance.this_t2.*.public_ip), list("")))))}"
